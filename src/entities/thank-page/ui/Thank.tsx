    ...
    <Card className="w-full max-w-full p-0 shadow-none border-none">
      <CardHeader className="px-0 pb-4 items-center text-center">
        ...
        <CardDescription className="text-sm text-gray-500">
          {t('order_id')} <span className="font-semibold text-gray-700">{displayOrderId}</span>
          {orderDate && <span className="text-gray-400"> · {orderDate}</span>}
        </CardDescription>
        {email && (
          <p className="text-sm text-gray-500">
            {t('email_sent_to')} <span className="font-medium text-gray-700">{email}</span>
          </p>
        )}
      </CardHeader>
      ...
      <CardContent className="space-y-3 px-0">
        ...
        <div className="flex items-start gap-3 rounded border border-green-100 bg-green-50/60 p-4">
          ...
          <div>
            <p className="text-xs font-medium text-green-800">{t('what_next_title')}</p>
            <p className="text-sm text-green-700 leading-relaxed mt-0.5">{t('what_next_text')}</p>
          </div>
        </div>
        ...
        {lineItems.length > 0 && (
          <div className="rounded border border-gray-100 bg-white">
            <p className="px-4 pt-3 pb-2 text-xs font-medium text-gray-400">{t('your_order')}</p>
              ...
              {lineItems.map(({ node: item }, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{t('quantity')}: {item.quantity}</p>
                  </div>
                  {item.variant?.price && (
              ...
              <div className="border-t border-gray-100 px-4 py-3 space-y-1.5 bg-gray-50/60">
                {subtotal && (
                  <div className="flex justify-between text-xs text-gray-500">
                    ...
                    <span>{Math.round(Number(subtotal.amount))} {getCurrencySymbol(subtotal.currencyCode)}</span>
                  </div>
                )}
                {shipping && (
                  <div className="flex justify-between text-xs text-gray-500">
                ...
                <div className="flex justify-between text-sm font-bold text-gray-900 pt-1 border-t border-gray-200">
                  <span>{t('total')}</span>
                  <span>{Math.round(Number(total.amount))} {currencySymbol}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <CardFooter className="flex flex-col sm:flex-row gap-2 px-0 pt-2">
        <Button asChild className="flex-1 h-10 bg-green-800 hover:bg-green-900 rounded">
