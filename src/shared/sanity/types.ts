...
// Query: *[_type == "post" && slug.current == $slug && (language == $language || !defined(language))][0]{  _id,  title,  body,  mainImage {    ...,    asset-> { _id, url, metadata { dimensions } },    alt  },  publishedAt,  language,  "slug": slug.current,  "categories": coalesce(    categories[]->{      _id,      slug,      title    },    []  ),  author->{    name,    image  },  relatedPosts[]{    _key,    ...@->{_id, title, slug, language, mainImage { ..., asset-> { _id, url, metadata { dimensions } }, alt }, publishedAt}  },  "seo": {  "title": coalesce(seo.title, title, ""),    "description": coalesce(seo.description,  ""),    "image": seo.image,    "noIndex": seo.noIndex == true   },  "translations": *[_type == "translation.metadata" && references(^._id)][0].translations[].value->{    "slug": slug.current,    language  },}
